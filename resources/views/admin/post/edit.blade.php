    ...
    <div class="col-12 card-primary">
        ...
        <form role="form" method="POST" action="{{ route('post.update', $post->id) }}" enctype="multipart/form-data">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    <label for="exampleInputPassword1">post Parent</label>
                    <select class="form-control" name="parent_cat_id">
                        <option value="">no post</option>
                        @foreach ($allData as $singlData)
                            <option value="{{ $singlData->id }}" @selected($singlData->id == $post->Category->id)>{{ $singlData->name }}
                            </option>
                        @endforeach
                    </select>
                </div>

                {{-- <x-dashboard.layouts.imgform oldimg='{{$post->image}}' pagename='post'/> --}}
